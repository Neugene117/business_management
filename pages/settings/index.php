...
$canManageTaxes = $canUpdateSettings && !isRolePreviewActive();
...
$taxes = [];
$taxStmt = mysqli_prepare($conn, 'SELECT id,name,tax_type,tax_value,is_active,created_at FROM taxes WHERE business_id=? ORDER BY is_active DESC,name,id');
if ($taxStmt) {
    mysqli_stmt_bind_param($taxStmt, 'i', $businessId);
    mysqli_stmt_execute($taxStmt);
    $taxResult = mysqli_stmt_get_result($taxStmt);
    while ($tax = mysqli_fetch_assoc($taxResult)) $taxes[] = $tax;
}
...
<div class="settings-shell">
  ...
  <nav class="settings-nav" aria-label="Settings sections" role="tablist">
    <a href="#company-identity" role="tab">Company</a><a href="#business-profile" role="tab">Business profile</a><a href="#accounting-settings" role="tab">Accounting</a><a href="#tax-registration" role="tab">Tax registration</a>
  ...
  <section class="card settings-section" id="accounting-settings">
    <div class="settings-section-heading"><div><h2>Accounting and inventory</h2><p>Core costing, fiscal year, and stock controls.</p></div></div>
  ...
  <section class="card settings-section" id="tax-registration">
    <div class="settings-section-heading"><div><h2>Tax registration</h2><p>Register percentage or fixed taxes and choose the one automatically applied to new sales.</p></div><?php $activeTaxCount=count(array_filter($taxes,static fn($tax)=>(int)$tax['is_active']===1)); ?><span class="settings-pill <?php echo $activeTaxCount ? 'ready-pill' : ''; ?>"><?php echo $activeTaxCount ? 'Tax active' : 'Sales tax off'; ?></span></div>
    <div class="tax-settings-layout">
      <form action="backend" method="POST" class="settings-form tax-create-form" id="taxForm">
        <input type="hidden" name="csrf_token" value="<?php echo e($csrfToken); ?>"><input type="hidden" name="action" value="create_tax">
        <div class="setting-field"><label for="tax-name">Tax name</label><input type="text" name="tax_name" id="tax-name" maxlength="100" placeholder="e.g. VAT" required <?php echo $canManageTaxes?'':'disabled'; ?>></div>
        <div class="setting-field"><label for="tax-type">Tax type</label><select name="tax_type" id="tax-type" required <?php echo $canManageTaxes?'':'disabled'; ?>><option value="PERCENTAGE">Percentage (%)</option><option value="FIXED">Fixed amount</option></select></div>
        <div class="setting-field"><label for="tax-value" id="tax-value-label">Percentage rate (%)</label><input type="number" name="tax_value" id="tax-value" min="0" max="100" step="0.0001" placeholder="18" required <?php echo $canManageTaxes?'':'disabled'; ?>></div>
        <label class="setting-check"><input type="checkbox" name="is_active" value="1" checked <?php echo $canManageTaxes?'':'disabled'; ?>><span><strong>Activate after registration</strong><small>This deactivates any tax currently applied to sales.</small></span></label>
        <?php if ($canManageTaxes): ?><div class="settings-form-actions full-field"><button class="btn-primary" type="submit">Register tax</button></div><?php endif; ?>
      </form>
      <div class="tax-list" aria-label="Registered taxes">
        <?php if (!$taxes): ?><div class="settings-empty-inline"><strong>No taxes registered</strong><p>New sales are currently recorded without tax.</p></div><?php endif; ?>
        <?php foreach ($taxes as $tax): ?>
          <article class="tax-item">
            <div class="tax-item-main"><span class="tax-type-icon"><?php echo $tax['tax_type']==='PERCENTAGE'?'%':e($business['currency_code']); ?></span><div><strong><?php echo e($tax['name']); ?></strong><p><?php echo $tax['tax_type']==='PERCENTAGE' ? e(rtrim(rtrim(number_format((float)$tax['tax_value'],4,'.',''),'0'),'.')).'%' : e(formatCurrency($tax['tax_value'],$business['currency_code'])); ?> &middot; <?php echo $tax['tax_type']==='PERCENTAGE'?'Percentage':'Fixed amount'; ?></p></div></div>
            <div class="tax-item-actions"><span class="settings-pill <?php echo (int)$tax['is_active']===1?'ready-pill':''; ?>"><?php echo (int)$tax['is_active']===1?'Active':'Inactive'; ?></span><?php if($canManageTaxes): ?><form action="backend" method="POST"><input type="hidden" name="csrf_token" value="<?php echo e($csrfToken); ?>"><input type="hidden" name="action" value="toggle_tax"><input type="hidden" name="tax_id" value="<?php echo (int)$tax['id']; ?>"><button type="submit" class="btn-secondary"><?php echo (int)$tax['is_active']===1?'Deactivate':'Activate'; ?></button></form><?php endif; ?></div>
          </article>
        <?php endforeach; ?>
      </div>
    </div>
    <div class="settings-note">Only one tax can be active at a time. When every tax is inactive, sales are completed without tax.</div>
  </section>
...
const taxType=document.getElementById('tax-type');const taxValue=document.getElementById('tax-value');const taxValueLabel=document.getElementById('tax-value-label');if(taxType&&taxValue&&taxValueLabel){function syncTaxType(){const percentage=taxType.value==='PERCENTAGE';taxValueLabel.textContent=percentage?'Percentage rate (%)':'Fixed amount (<?php echo e($business['currency_code']); ?>)';taxValue.max=percentage?'100':'';taxValue.placeholder=percentage?'18':'500';}taxType.addEventListener('change',syncTaxType);syncTaxType();}
