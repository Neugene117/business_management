...
$returnParams = [];
foreach (['search', 'status', 'page'] as $key) {
...
<main class="purchase-record-page">
  ...
  <header class="record-header">
    <div><span class="record-eyebrow">Purchase order</span><h1><?php echo e($purchase['purchase_number']); ?></h1><p><?php echo e($purchase['supplier_name']); ?> &middot; Ordered <?php echo e(formatDate($purchase['purchase_date'], $config['timezone'], 'd M Y, H:i')); ?></p></div>
  ...
  <section class="record-summary" aria-label="Purchase summary">
    <article><span>Order Total</span><strong><?php echo formatCurrency($purchase['total_amount'], $currency); ?></strong><small><?php echo count($items); ?> product line(s)</small></article>
  ...
  <div class="record-layout">
    <div class="record-main">
      ...
      <div class="record-activity-grid">
        <section class="record-section"><div class="record-section-heading"><div><h2>Payment History</h2><p>Recorded payments and method-specific details.</p></div><span><?php echo count($payments); ?> records</span></div><?php if(!$payments):?><div class="record-empty"><strong>Recorded as supplier debt</strong><p>No payment has been recorded. The outstanding balance is <?php echo formatCurrency($balanceDue,$currency); ?>.</p></div><?php else:?><div class="record-list"><?php foreach($payments as $payment):$methodLabel=$payment['payment_method']==='MOBILE_MONEY'?'Phone / Mobile Money':($payment['payment_method']==='BANK_TRANSFER'?'Bank Transfer':ucwords(strtolower(str_replace('_',' ',$payment['payment_method']))));?><article><div><strong><?php echo e($methodLabel); ?></strong><small><?php echo e(formatDate($payment['paid_at'],$config['timezone'],'d M Y, H:i')); ?> &middot; <?php echo e(trim((string)$payment['recorded_by'])?:'System'); ?></small><p><?php if($payment['payment_method']==='MOBILE_MONEY'):?>Telephone: <?php echo e($payment['phone_number']?:'Not recorded'); ?><?php elseif($payment['payment_method']==='BANK_TRANSFER'):?>Bank: <?php echo e($payment['bank_name']?:'Not recorded'); ?> &middot; Account: <?php echo e($payment['bank_account_number']?:'Not recorded'); ?><?php else:?>Cash payment<?php endif;?><?php if($payment['reference_number']):?> &middot; Reference: <?php echo e($payment['reference_number']); ?><?php endif;?><?php if($payment['notes']):?> &middot; <?php echo e($payment['notes']); ?><?php endif;?></p></div><b><?php echo formatCurrency($payment['amount'],$currency); ?></b></article><?php endforeach;?></div><?php endif;?></section>
